try:
    import lmdb
except ImportError:  # lmdb 仅用于 LaSOT/GOT-10k 等 lmdb 格式训练数据，RGBT 测试不需要
    lmdb = None
import numpy as np
import cv2
import json

_LMDB_UNAVAILABLE_MSG = (
    'lmdb 未安装，无法读取 lmdb 格式数据集。'
    'RGBT/LasHeR 测试不需要 lmdb；如需使用 lmdb 训练数据，请安装: pip install lmdb'
)
...
def get_lmdb_handle(name):
    if lmdb is None:
        raise RuntimeError(_LMDB_UNAVAILABLE_MSG)
