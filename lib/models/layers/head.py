...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        center_head = CenterPredictor(inplanes=in_channel, channel=out_channel,
                                      feat_sz=feat_sz, stride=stride)
        return center_head
    elif cfg.MODEL.HEAD.TYPE == "CONDITIONAL_CENTER":
        from lib.models.layers.conditional_head import build_conditional_head
        return build_conditional_head(cfg, hidden_dim)
    else:
