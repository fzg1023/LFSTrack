...
from lib.train.admin.model_ema import ModelEma
import torch
import time
...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        self.use_ema = getattr(settings, 'use_ema', False)
        if self.use_ema:
            ema_decay = getattr(settings, 'ema_decay', 0.9998)
            self.ema = ModelEma(self.actor.net, decay=ema_decay)
    ...
    def cycle_dataset(self, loader):
        ...
        # Swap to EMA weights once before the entire eval epoch
        if not loader.training and self.use_ema:
        ...
        # Restore original weights after eval epoch
        if not loader.training and self.use_ema:
