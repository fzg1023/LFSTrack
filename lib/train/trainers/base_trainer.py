...
class BaseTrainer:
    ...
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None):
        ...
        # Optional: run full test after each epoch
        self.test_cmd = getattr(settings, 'test_cmd', None)
        self.test_dataset = getattr(settings, 'test_dataset', 'lasher')
    ...
    def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False):
        ...
        for i in range(num_tries):
            try:
                ...
                # Record resume epoch for cosine annealing offset
                cosine_start_epoch = self.epoch
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if self.lr_scheduler is not None:
                        if self.settings.scheduler_type != 'cosine':
                            ...
                            self.lr_scheduler.step(epoch - 1 - cosine_start_epoch)
                    ...
                    if epoch > (max_epochs - save_last_n_epoch) or epoch % save_epoch_interval == 0 or epoch >4:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                ...
                                # ── Run full test in background after saving ──
                                if self.test_cmd is not None:
                                    test_interval = getattr(self.settings, 'test_interval', 1)
                                    if epoch % test_interval == 0:
                                        self._launch_test(epoch)
            except:
    ...
    def _launch_test(self, epoch):
        """Run full test evaluation synchronously after saving checkpoint."""
        net = self.actor.net.module if multigpu.is_multi_gpu(self.actor.net) else self.actor.net
        net_type = type(net).__name__
        directory = '{}/{}'.format(self._checkpoint_dir, self.settings.project_path)
        ckpt_path = '{}/{}_ep{:04d}.pth.tar'.format(directory, net_type, epoch)

        prj_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../..'))
        test_script = os.path.join(prj_dir, 'RGBT_workspace', 'test_uatrack.py')
        cmd = [
            sys.executable, test_script,
            '--script', self.settings.script_name,
            '--config', self.settings.config_name,
            '--checkpoint', ckpt_path,
            '--dataset', self.test_dataset,
            '--workers', '1',
        ]
        print(f'[Test] Running test for epoch {epoch}...')
        subprocess.run(cmd)
        print(f'[Test] Epoch {epoch} test complete.')
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        for key in fields:
            if key in ignore_fields:
                continue
            if key == 'net':
                from lib.models.bat.ostrack_adapter import remap_legacy_keys
                net.load_state_dict(remap_legacy_keys(checkpoint_dict[key]), strict=False)
            elif key == 'optimizer':
                try:
                    self.optimizer.load_state_dict(checkpoint_dict[key])
                except ValueError:
                    print('[WARNING] Optimizer state incompatible (model changed). Starting with fresh optimizer.')
            else:
    ...
    def load_state_dict(self, checkpoint=None, distill=False):
        ...
        from lib.models.bat.ostrack_adapter import remap_legacy_keys
        missing_k, unexpected_k = net.load_state_dict(remap_legacy_keys(checkpoint_dict["net"]), strict=False)
