...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
        ...
        # ── Temporal (multi-frame): sample CONSECUTIVE frames ──
        if num_ids > 1:
            if len(valid_ids) < num_ids:
                return None
            # Random start, then take consecutive frames
            max_start = len(valid_ids) - num_ids
            start = random.randint(0, max_start)
            return [valid_ids[i] for i in range(start, start + num_ids)]
